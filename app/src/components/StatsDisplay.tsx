...
export const StatsDisplay: React.FC<StatsDisplayProps> = ({
  ...
  return (
    <div className={styles.statsContainer}>
      ...
      {tokenMetadata !== undefined ? (
        <>
          {new Intl.NumberFormat('en-US', { notation: 'compact' }).format(
            Math.floor(Number(contractState.fields.balanceBurn) / 10 ** tokenMetadata.decimals)
          )} {tokenMetadata.symbol}
          {exPrice > 0 && (
            <span className={styles.usdPrice}>
              (≈${((Number(contractState.fields.balanceBurn) / 10 ** tokenMetadata.decimals) * burnRatio * exPrice).toFixed(2)})
            </span>
          )}
        </>
      ) : (
      ...
      <span className={styles.mintCount}>
      Minted: 
