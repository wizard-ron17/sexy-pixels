...
export const TokenDapp: FC<{
  ...
  const [pixels, setPixels] = useState<Pixel[]>(
    Array(gridSize * gridSize).fill({ color: "#333" })
  );
  ...
  const [pendingPixelClick, setPendingPixelClick] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    async function subscribeEvent() {
      const currentCount =
        await contractFactory.getContractEventsCurrentCount();
  ...
  useEffect(() => {
    if (
      ...
      const tokenBalanceWallet = balance.tokenBalances?.find(
        (token: { id: string }) =>
          token.id === contractState.fields.tokenIdToBurn
      );
  ...
  const handleResetSubmit = async () => {
    ...
    try {
      if (signer && contractState !== null) {
        if (tokenBalance !== undefined && tokenMetadata) {
          const requiredAmount = Number(
            contractState.fields.resetBurnMultiplier *
              contractState.fields.burnMint
          );
          if (tokenBalance < requiredAmount) {
            setInsufficientTokens(true);
      ...
      pixels.map((pixel, index) => (
        <div
          ...
          className={`${gridStyles.pixel} ${
            pixel.isShiny ? gridStyles.blink : ""
          }`}
    ...
    if (selectedPixel !== null && selectedColor) {
      ...
      if (contractState && tokenBalance !== undefined && tokenMetadata) {
        const requiredAmount =
          ...
          Number(contractState.fields.shinyMultiplier); // 10x for shiny
        if (tokenBalance < requiredAmount) {
          setInsufficientTokens(true);
          return;
        }
      ...
      {modalVisible && !isResetModal && (
        <div className={gridStyles.modal}>
          <div className={gridStyles.modalContent}>
            ...
            <p>
              Contract Fee: <strong>0.1 ALPH</strong>
            </p>
            ...
            <p>
              <small>
                Shiny pixels burn{" "}
                {Number(contractState?.fields.shinyMultiplier)}x more than
                normal pixels:{" "}
                {contractState !== null && tokenMetadata !== undefined
                  ? `${
                      (Number(contractState.fields.burnMint) /
                        10 ** tokenMetadata.decimals) *
                      Number(contractState?.fields.shinyMultiplier)
                    } ${tokenMetadata?.symbol}`
                  : "0"}
              </small>
            </p>
            ...
            <button
              id="makeItShineButton"
              onClick={handleShinyColorSubmit}
              disabled={!selectedColor}
            >
      ...
      {modalVisible && isResetModal && (
        <div className={gridStyles.modal}>
          <div className={gridStyles.modalContent}>
            ...
            <p>
              <b>
                Resetting pixels burn{" "}
                {contractState !== null && tokenMetadata !== undefined
                  ? `${
                      (Number(contractState.fields.burnMint) /
                        10 ** tokenMetadata.decimals) *
                      Number(contractState?.fields.resetBurnMultiplier)
                    } ${tokenMetadata?.symbol}`
                  : "0"}
              </b>
            </p>
          ...
          <div className={gridStyles.modalContent}>
            ...
            <p>
              ...
              {tokenBalance && tokenMetadata
                ? `${tokenBalance / 10 ** tokenMetadata.decimals} ${
                    tokenMetadata.symbol
                  }`
                : "0"}
            </p>
            <button
              onClick={() => {
                if (contractState?.fields.tokenIdToBurn) {
                  window.open(
                    `https://www.elexium.finance/swap?tokenA=tgx7VNFoP9DJiFMFgXXtafQZkUvyEdDHT9ryamHJYrjq&tokenB=${addressFromTokenId(
                      contractState.fields.tokenIdToBurn
                    )}`,
                    "_blank"
                  );
